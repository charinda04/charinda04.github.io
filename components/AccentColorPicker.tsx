...
export const AccentColorPicker: React.FC<AccentColorPickerProps> = ({
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div 
          className="w-5 h-5 rounded-full border border-theme-border"
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 p-2 bg-theme-card-bg border border-theme-border rounded-lg shadow-minimal-lg backdrop-blur-sm z-50">
          <div className="flex flex-col gap-1.5">
            {ACCENT_COLORS.map((color) => (
              <button
                key={color.id}
                onClick={() => handleColorSelect(color.id)}
                className={`w-6 h-6 rounded-full border transition-all duration-150 hover:scale-105 ${
